...
# Size of source mod 2**32: 40537 bytes
import functools
...
class CarrySystemTerrainTarget(CarrySystemTarget):
    ...
    def get_constraint(self, sim, **kwargs):
        constraint = (self._obj.get_pick_up_constraint)(sim, (self._route_target.position), 
         (self._route_target.routing_surface), **kwargs)
...
class CarrySystemRuntimeSlotTarget(CarrySystemCustomAnimationTarget):
 ...
 L. 274         0  LOAD_FAST                'self'
                2  LOAD_ATTR                _put
                4  POP_JUMP_IF_FALSE    22  'to 22'

 L. 275         6  LOAD_FAST                'self'
...
class CarryingObject(CarryPosture):
    ...
    def get_good_location_on_floor_for_jig(jig_definition, starting_transform, starting_routing_surface):
        ...
        fgl_context = placement.FindGoodLocationContext(starting_location, object_def_id=(jig_definition.id),
          object_footprints=(
         footprint,),
          max_results=1,
