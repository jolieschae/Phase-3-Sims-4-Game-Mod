...
# Size of source mod 2**32: 23753 bytes
import functools
...
def get_carried_objects_gen(sim):
    ...
    if posture_back is not None:
        if posture_back.target is not None:
            yield (
             Hand.BACK, posture_back, posture_back.target)
...
def create_carry_nothing_constraint(hand, debug_name='CarryNothing'):
    ...
    if hand == Hand.LEFT:
        entries = (
         PostureManifestEntry(None, MATCH_ANY, MATCH_ANY, MATCH_ANY, MATCH_NONE, MATCH_ANY, MATCH_ANY, MATCH_ANY),)
    else:
        if hand == Hand.RIGHT:
            entries = (
             PostureManifestEntry(None, MATCH_ANY, MATCH_ANY, MATCH_ANY, MATCH_ANY, MATCH_NONE, MATCH_ANY, MATCH_ANY),)
        else:
            entries = (
             PostureManifestEntry(None, MATCH_ANY, MATCH_ANY, MATCH_ANY, MATCH_ANY, MATCH_ANY, MATCH_NONE, MATCH_ANY),)
