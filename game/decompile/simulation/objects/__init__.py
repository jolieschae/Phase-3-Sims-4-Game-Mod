...
# Size of source mod 2**32: 14684 bytes
...
class PuzzleState(collections.namedtuple('PuzzleState', PaintingState._fields + ('reveal_texture_id_b',
                                                               'is_linked_canvas')), PaintingState):
    REVEAL_LEVEL_MAX = 8
    ...
    def __new__(cls, texture_id, reveal_level=0, use_overlay=False, effect=sims4.math.MAX_UINT32, stage_texture_id=None, overlay_texture_id=None, reveal_texture_id=None, reveal_texture_id_b=None, is_linked_canvas=False):
        if reveal_level < cls.REVEAL_LEVEL_MIN or reveal_level > cls.REVEAL_LEVEL_MAX:
            raise ValueError('reveal_level ({}) is out of range [{} - {}].'.format(reveal_level, cls.REVEAL_LEVEL_MIN, cls.REVEAL_LEVEL_MAX))
        if not isinstance(texture_id, int):
            raise TypeError('texture_id must be an integer.')
