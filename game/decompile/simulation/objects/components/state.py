...
# Size of source mod 2**32: 242742 bytes
import itertools
from protocolbuffers import SimObjectAttributes_pb2 as protocols
import alarms, caches, clock, collections, enum, event_testing, gsi_handlers, objects, operator, placement, random, services, sims4.log, weakref
...
class OptionalTunableClientStateChangeItemWithDisable(OptionalTunable):
    ...
    def __init__(self, tunable, tunable_disabled_name=None, tunable_enabled_name=None, **kwargs):
        ...
        (super().__init__)(disabled_value=UNSET, disabled_name='leave_unchanged', 
         enabled_name='apply_new_value', 
         tunable=OptionalTunable(tunable=tunable, disabled_name=tunable_disabled_name,
  enabled_name=tunable_enabled_name), **kwargs)
...
class StatisticModifierList(HasTunableFactory, AutoFactoryInit):
    ...
    FACTORY_TUNABLES = {'autonomy_modifiers':TunableList(description='\n            List of possible Modifiers that may happen when a statistic gets \n            hit, this will modify the objects statistics behavior.\n            ',
     ...
     'periodic_statistic_change':TunableTuple(description='\n            The stat change apply on the target within the state value.\n            ',
       ...
       default=60),
       operations=TunableList(tunable=(ObjectStatisticChangeOp.TunableFactory()))), 
     'lot_statistic_modifiers':TunableList(description='\n            A list of continuous statistic modifiers to apply on the lot.\n            ',
...
class StateSituationRequest(HasTunableFactory, AutoFactoryInit):
    ...
    def start(self):
        ...
        for situation_info in self.situations_to_create:
            ...
            situation_id = situation_manager.create_situation(situation_type=situation, guest_list=guest_list,
              user_facing=False)
...
class UiMetadataList(HasTunableFactory, AutoFactoryInit, NeedsStateValue):
    FACTORY_TUNABLES = {'data': TunableMapping(description='\n        ',
               key_type=str,
               value_type=TunableVariant(other_value=TunableVariant(default='integral',
               boolean=(Tunable(bool, False)),
               string=(TunableLocalizedString()),
               integral=(Tunable(int, 0)),
               icon=(TunableIcon()),
               color=(TunableColor())),
               state_value=TunableVariant(default='value',
               locked_args={
...
class ObjectReplacementOperation(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def __call__(self, target):
        if self.new_object is None:
                ...
                if target.in_use:
                    ...
                    if self.use_fgl:
                        ...
                        if new_location is None:
                            ...
                            fgl_context = placement.create_fgl_context_for_object(starting_location, (self.new_object), search_flags=search_flags,
                              ignored_object_ids=(
                             target.id,))
            ...
            elif self.transfer_sim_ownership:
                if target.ownable_component is not None:
                    if created_obj.ownable_component is not None:
...
class ObjectValueChangeOperation(HasTunableFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {'value_change_type':TunableVariant(decrease_value=ValueDecreaseFactory.TunableFactory(), increase_value=ValueIncreaseFactory.TunableFactory(),
       default='decrease_value'), 
     'change_percentage':TunablePercent(description='\n            A percentage of the catalog value to modify the current value of \n            the target. It will either decrease or increase the value of the \n            object based on the setting of reduce_value.\n            ',
       default=100)}
        ...
        if in_inventory:
            if attr_name in StateChangeOperation.INVENTORY_AFFECTED_DISTRIBUTABLES:
                ...
                if forward_to_owner:
                    ...
                    return False
        if self._client_state_suppressors:
...
class StateChangeOperation(HasTunableSingletonFactory):
    FACTORY_TUNABLES = {'tint': OptionalTunableClientStateChangeItem(tunable=TunableColorSnippet(description='A tint to apply')), 
     
     'opacity': OptionalTunableClientStateChangeItem(tunable=TunableRange(float, 1, 0, 1, description='An opacity to apply')), 
     
     'scale': OptionalTunableClientStateChangeItem(tunable=(ScaleOperation.TunableFactory())), 
     
     'visibility': OptionalTunableClientStateChangeItem(tunable=TunableVisibilityState(description='A visibility state to apply')), 
     
     'visibility_flags': OptionalTunableClientStateChangeItem(description='\n            If specified, apply visibility flag overrides for this object. For\n            example, control whether or not the object is reflected in mirrors\n            and water.\n            ',
                           tunable=OptionalTunable(tunable=TunableEnumFlags(enum_type=VisibilityFlags,
                           allow_no_flags=True),
                           disabled_name='No_Flags')), 
     
     StateChange.MODEL_SUITE_STATE_INDEX: OptionalTunableClientStateChangeItem(description='\n            For object definitions that use a suite of models (each w/ its own\n            model, rig, slots, slot resources, and footprint), switch the index\n            used in the suite.\n            ',
                                            tunable=TunableModelSuiteStateIndex(description='Index to use.')), 
     
     'geometry_state': OptionalTunableClientStateChangeItem(tunable=TunableGeometryState(description='A geometry state to apply')), 
     
     StateChange.GEOMETRY_STATE_OVERRIDE: OptionalTunableClientStateChangeItem(description='\n            Apply a geometry state override to the object.\n            ',
                                            tunable=(GeometryStateOverrideOperation.TunableFactory())), 
     
     'material_state': OptionalTunableClientStateChangeItem(tunable=TunableMaterialState(description='A material state to apply')), 
     
     'model': OptionalTunableClientStateChangeItem(tunable=TunableModelOrDefault(description='A model state to apply')), 
     
     'material_variant': OptionalTunableClientStateChangeItem(tunable=TunableMaterialVariant('materialVariantName', description='A material variant to apply')), 
     
     'pregnancy_progress': OptionalTunableClientStateChangeItem(tunable=TunableRange(float, 0, 0, 1, description='A pregnancy progress value to apply')), 
     ...
     StateChange.PAINTING_REVEAL_LEVEL: OptionalTunableClientStateChangeItem(tunable=TunableRange(description='\n                A canvas reveal level to apply.  Smaller values show less of\n                the final canvas.  The maximum value fully reveals the\n                canvas. NOTE: Painting reveal levels cannot be greater than 5.\n                ',
                                          tunable_type=int,
                                          default=(PaintingState.REVEAL_LEVEL_MIN),
                                          minimum=(PaintingState.REVEAL_LEVEL_MIN),
                                          maximum=(PuzzleState.REVEAL_LEVEL_MAX))), 
     
     StateChange.PAINTING_STATE: OptionalTunableClientStateChangeItem(description='\n            Change the entire painting state.\n            ',
                                   tunable=(ChangePaintingState.TunableFactory())), 
     
     StateChange.FLOWING_PUDDLE_ENABLED: OptionalTunableClientStateChangeItem(tunable=Tunable(bool, False, description='If True, this object will start spawning puddles based on its PuddleSpawningComponentTuning.')), 
     
     StateChange.VFX: OptionalTunableClientStateChangeItemWithDisable(description='\n            Play one or more visual effects.\n            ',
                        tunable=TunableVariant(description='\n                Define the type of visual effect to play.\n                ',
                        single_effect=(PlayEffect.TunableFactory()),
                        multiple_effects=(PlayMultipleEffects.TunableFactory()),
                        default='single_effect'),
                        tunable_disabled_name='no_vfx',
                        tunable_enabled_name='start_vfx'), 
     
     StateChange.VFX_STATE: OptionalTunableClientStateChangeItem(description='\n            Define a visual effect state on any visual effects controlled by\n            this state.\n            ',
                              tunable=(PlayEffectState.TunableFactory())), 
     
     StateChange.GLOW: OptionalTunableClientStateChangeItemWithDisable(tunable=(Glow.TunableFactory()),
                         tunable_disabled_name='no_glow',
                         tunable_enabled_name='start_glow'), 
     
     StateChange.AUDIO_STATE: OptionalTunableClientStateChangeItemWithDisable(tunable=TunablePlayAudio(description='An audio state to apply'),
                                tunable_disabled_name='no_audio',
                                tunable_enabled_name='start_audio'), 
     
     StateChange.AUDIO_EFFECT_STATE: OptionalTunableClientStateChangeItemWithDisable(description='\n            A way to apply An audio effect (.effectx) to the object when state changes\n            ',
                                       tunable=(ApplyAudioEffect.TunableFactory()),
                                       tunable_disabled_name='no_audio_effect',
                                       tunable_enabled_name='start_audio_effect'), 
     
     StateChange.VIDEO_STATE: OptionalTunableClientStateChangeItem(tunable=OptionalTunable(disabled_name='no_video',
                                enabled_name='start_video',
                                tunable=TunableTuple(description='\n                    List of clip names and append behavior.\n                    ',
                                clip_list=(TunableList(TunableResourceKey(None, resource_types=[RESOURCE_TYPE_VP6]))),
                                append_clip=Tunable(description='\n                        If enabled clip list will be appended to previous \n                        playing clip instead of interrupting the existing \n                        playlist.\n                        This should be tuned when the clips should have a \n                        smoother transition like a credits scene on a movie.\n                        ',
                                tunable_type=bool,
                                default=False),
                                loop_last=Tunable(description="\n                        If enabled, the clip will loop. Otherwise, it's played\n                        once and stop on last frame.\n                        ",
                                tunable_type=bool,
                                default=True)))), 
     
     StateChange.VIDEO_STATE_LOOPING: OptionalTunableClientStateChangeItemWithDisable(tunable=TunableResourceKey(None, resource_types=[sims4.resources.Types.PLAYLIST]),
                                        tunable_disabled_name='no_video',
                                        tunable_enabled_name='start_video'), 
     
     'transient': OptionalTunableClientStateChangeItem(tunable=Tunable(bool, False, description='This is what the objects transient value is set to')), 
     
     StateChange.AUTONOMY_MODIFIERS: OptionalTunableClientStateChangeItemWithDisable(tunable=(StatisticModifierList.TunableFactory()),
                                       tunable_disabled_name='no_statistic_to_apply',
                                       tunable_enabled_name='apply_statistic_modifiers'), 
     
     StateChange.AWARENESS: OptionalTunableClientStateChangeItemWithDisable(tunable=(AwarenessSourceRequest.TunableFactory()),
                              tunable_disabled_name='no_awareness',
                              tunable_enabled_name='start_awareness'), 
     
     StateChange.BROADCASTER: OptionalTunableClientStateChangeItem(tunable=OptionalTunable(tunable=BroadcasterRequest.TunableFactory(locked_args={'participant':None, 
                               'offset_time':None}),
                                disabled_name='no_broadcaster',
                                enabled_name='start_broadcaster')), 
     
     StateChange.ROUTE_EVENT: OptionalTunableClientStateChangeItem(tunable=OptionalTunable(tunable=RouteEventProviderRequest.TunableFactory(locked_args={'participant': None}))), 
     
     StateChange.WALKSTYLE: OptionalTunableClientStateChangeItem(tunable=OptionalTunable(tunable=(WalkStyleRequest.TunableFactory()),
                              disabled_name='no_request',
                              enabled_name='request_walkstyle')), 
     
     StateChange.REPLACE_OBJECT: OptionalTunable(disabled_value=UNSET,
                                   tunable=(ObjectReplacementOperation.TunableFactory())), 
     
     StateChange.TOGGLE_FOOTPRINT: OptionalTunableClientStateChangeItem(tunable=(ToggleFootprintOperation.TunableFactory())), 
     
     StateChange.UI_METADATA: OptionalTunableClientStateChangeItem(tunable=(UiMetadataList.TunableFactory())), 
     
     StateChange.LIGHT_DIMMER_STATE: OptionalTunableClientStateChangeItem(tunable=TunableRange(float, 0, 0, 1, description='A dimmer value to apply')), 
     
     StateChange.LIGHT_MATERIAL_STATE: OptionalTunableClientStateChangeItem(description='\n            Override the material states to apply on the object when its light\n            is on or off.\n            ',
                                         ...
                                         tunable_type=str,
                                         default='lightson'),
                                         locked_args={'set_to_default':DEFAULT, 
                                        'leave_unchanged':None},
                                         default='leave_unchanged'),
                                         material_state_off=TunableVariant(description='\n                    If specified, override the material state for this light\n                    when the object is off.\n                    ',
     ...
     StateChange.LIGHT_HAUNTED_STATE: OptionalTunableClientStateChangeItem(tunable=Tunable(tunable_type=bool, default=True, description='Whether the light should be flickering.')), 
     
     StateChange.LOT_MODIFIERS: OptionalTunableClientStateChangeItemWithDisable(tunable=(LotStatisticModifierList.TunableFactory()),
                                  tunable_disabled_name='no_modifiers',
                                  tunable_enabled_name='apply_modifiers'), 
     
     StateChange.SINGED: OptionalTunableClientStateChangeItem(tunable=Tunable(tunable_type=bool, default=True)), 
     
     StateChange.WIND_SPEED_LEVEL: OptionalTunableClientStateChangeItem(tunable=(WindSpeedEffect.TunableFactory())), 
     
     StateChange.GRUBBY: OptionalTunableClientStateChangeItem(tunable=Tunable(tunable_type=bool, default=True)), 
     
     StateChange.DYED: OptionalTunableClientStateChangeItem(tunable=Tunable(tunable_type=bool, default=True)), 
     
     StateChange.MESSY_FACE: OptionalTunableClientStateChangeItem(tunable=Tunable(tunable_type=bool, default=True)), 
     
     StateChange.TAN_LEVEL: OptionalTunableClientStateChangeItem(tunable=(ChangeTanLevel.TunableFactory())), 
     
     StateChange.SCRATCHED: OptionalTunableClientStateChangeItem(description='\n            Change the state of this object appearing scratched.\n            ',
                              tunable=Tunable(tunable_type=bool, default=True)), 
     
     StateChange.CHANGE_VALUE: OptionalTunable(disabled_value=UNSET,
                                 tunable=(ObjectValueChangeOperation.TunableFactory())), 
     
     StateChange.MANNEQUIN_POSE: OptionalTunableClientStateChangeItem(tunable=(ObjectPose.TunableReference())), 
     
     StateChange.UTILITY_MODIFIERS: OptionalTunableClientStateChangeItem(tunable=(UtilityModifierState.TunableFactory())), 
     
     StateChange.LIVE_DRAG: OptionalTunableClientStateChangeItem(tunable=(LiveDragStateOperation.TunableFactory())), 
     
     StateChange.FOCUS_SCORE: OptionalTunableClientStateChangeItem(tunable=(TunableFocusScoreVariant())), 
     
     StateChange.PERIODIC_LOOT: OptionalTunableClientStateChangeItem(tunable=(PeriodicLootOperation.TunableFactory())), 
     
     StateChange.SITUATION: OptionalTunableClientStateChangeItem(description='\n            Create situations while this state is active.\n            ',
                              tunable=(StateSituationRequest.TunableFactory())), 
     
     StateChange.SET_FIRE: OptionalTunableClientStateChangeItem(tunable=(SetFireState.TunableFactory())), 
     
     StateChange.VOICE_ACTOR: OptionalTunableClientStateChangeItem(tunable=(SetObjectVoiceActorState.TunableFactory())), 
     
     StateChange.SLOTS: OptionalTunableClientStateChangeItem(tunable=(SetSlotsState.TunableFactory()))}
    CUSTOM_DISTRIBUTABLE_CHANGES = (
     StateChange.AUDIO_EFFECT_STATE,
     StateChange.AUDIO_STATE,
     StateChange.AUTONOMY_MODIFIERS,
     StateChange.AWARENESS,
     StateChange.BROADCASTER,
     StateChange.ENVIRONMENT_SCORE,
     StateChange.REPLACE_OBJECT,
     StateChange.TOGGLE_FOOTPRINT,
     StateChange.UI_METADATA,
     StateChange.VFX,
     StateChange.VFX_STATE,
     StateChange.LOT_MODIFIERS,
     StateChange.CHANGE_VALUE,
     StateChange.GEOMETRY_STATE_OVERRIDE,
     StateChange.MODEL_SUITE_STATE_INDEX,
     StateChange.UTILITY_MODIFIERS,
     StateChange.PAINTING_STATE,
     StateChange.LIVE_DRAG,
     StateChange.WALKSTYLE,
     StateChange.PERIODIC_LOOT,
     StateChange.GLOW,
     StateChange.SITUATION,
     StateChange.TAN_LEVEL,
     StateChange.ROUTE_EVENT,
     StateChange.SET_FIRE,
     StateChange.SCALE,
     StateChange.VOICE_ACTOR,
     StateChange.SLOTS)
    POST_LOAD_ENABLED_DISTRIBUTABLES = (
     StateChange.AUDIO_EFFECT_STATE,
     StateChange.AUDIO_STATE,
     StateChange.REPLACE_OBJECT,
     StateChange.TOGGLE_FOOTPRINT,
     ...
     StateChange.VOICE_ACTOR,
     StateChange.SLOTS)
    INVENTORY_AFFECTED_DISTRIBUTABLES = (
     StateChange.AUDIO_EFFECT_STATE,
     StateChange.AUDIO_STATE,
     StateChange.VFX,
     StateChange.GLOW)
    USE_COMPONENT_FOR = {StateChange.PAINTING_REVEAL_LEVEL: CANVAS_COMPONENT.instance_attr, 
     StateChange.FLOWING_PUDDLE_ENABLED: FLOWING_PUDDLE_COMPONENT.instance_attr, 
     StateChange.VIDEO_STATE: VIDEO_COMPONENT.instance_attr, 
     StateChange.VIDEO_STATE_LOOPING: VIDEO_COMPONENT.instance_attr, 
     StateChange.LIGHT_DIMMER_STATE: LIGHTING_COMPONENT.instance_attr, 
     StateChange.LIGHT_MATERIAL_STATE: LIGHTING_COMPONENT.instance_attr, 
     StateChange.LIGHT_HAUNTED_STATE: LIGHTING_COMPONENT.instance_attr, 
     StateChange.MANNEQUIN_POSE: MANNEQUIN_COMPONENT.instance_attr, 
     StateChange.FOCUS_SCORE: FOCUS_COMPONENT.instance_attr}
    ...
    def apply(self, target, custom_distributables, state, state_value, immediate=False, post_load_distributable_only=False, attr_args_dict=None):
        ...
        for attr_name, attr_value in self.ops.items():
            if attr_name in self.CUSTOM_DISTRIBUTABLE_CHANGES:
                if post_load_distributable_only:
                    if attr_name not in self.POST_LOAD_ENABLED_DISTRIBUTABLES:
                        continue
                apply_args = {}
                if attr_args_dict is not None:
...
class ObjectStateValue(HasTunableReference, ObjectStateValueDisplayMixin, SuperAffordanceProviderMixin, metaclass=ObjectStateMetaclass, manager=services.get_instance_manager(sims4.resources.Types.OBJECT_STATE)):
    INSTANCE_TUNABLES = {'value':TunableVariant(locked_args={'unordered': None},
       boolean=Tunable(bool, True),
       integral=Tunable(int, 0),
       decimal=Tunable(float, 0)), 
     'anim_overrides':OptionalTunable(TunableAnimationObjectOverrides(description='\n            Tunable class to contain param/vfx/props overrides\n            ')), 
...
class CommodityBasedObjectStateValue(ObjectStateValue):
    ...
    INSTANCE_TUNABLES = {'range':TunableInterval(description="\n            The commodity range this state maps to. The ranges between the commodity\n            values must have some overlap in order for the state to transition properly.\n            For instance, let's say you have two states, DIRTY and CLEAN. If you set the\n            DIRTY state to have a range between 0 and 20, and you set CLEAN state to have\n            a range of 21 to 100, the states will not change properly because of the void\n            created (between 20 and 21). At the very least, the lower bounds of one needs\n            to be the same as the upper bound for the next (i.e. DIRTY from 0 to 20 and\n            CLEAN from 20 to 100).\n            ",
       ...
       default_lower=0,
       default_upper=0), 
     'default_value':OptionalTunable(Tunable(description='\n                default commodity value when set to this state.\n                If disabled use average of range',
       tunable_type=float,
       default=0),
       disabled_name='use_range_average',
       enabled_name='use_default_value')}
...
class VideoChannel(ChannelBasedObjectStateValue):
    INSTANCE_TUNABLES = {'affordance': TunableReference(manager=(services.get_instance_manager(sims4.resources.Types.INTERACTION)),
                     allow_none=True)}
...
class AudioChannel(ChannelBasedObjectStateValue):
    ...
    def on_interaction_canceled_from_state_change(cls, interaction):
        ...
        stereo_component = interaction.target.get_component(STEREO_COMPONENT)
        return stereo_component is None or stereo_component.is_stereo_turned_on() or None
...
class TestedStateValueReference(HasTunableSingletonFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {'tested_states':TunableList(description='\n            The first test that passes will have its state applied.\n            ',
       tunable=TunableTuple(tests=(event_testing.tests.TunableTestSet()), state=TunableStateValueReference(pack_safe=True))), 
     'fallback_state':OptionalTunable(description='\n            If all tests fail, this state will be applied.\n            ',
...
class ObjectState(HasTunableReference, ObjectStateValueDisplayMixin, metaclass=ObjectStateMetaclass, manager=services.get_instance_manager(sims4.resources.Types.OBJECT_STATE)):
    ...
    def _verify_tuning_callback(cls):
        if cls.linked_stat is not None:
            if cls.persists_across_gallery != cls.linked_stat.persists_across_gallery_for_state:
...
class CommodityBasedObjectState(ObjectState):
    INSTANCE_TUNABLES = {'linked_stat':TunableReference(description='\n            The statistic to link to the state.\n            ',
     ...
     '_values':TunableList(tunable=TunableStateValueReference(class_restrictions=CommodityBasedObjectStateValue),
       unique_entries=True,
       minlength=1), 
     'lot_based':Tunable(description='\n            Whether the state should check the linked stat on the active lot\n            instead of on the object itself.\n            ',
       tunable_type=bool,
       default=False)}
    ...
    def _verify_tuning_callback(cls):
        ...
        if not cls.linked_stat.use_stat_value_on_initialization:
            if cls.linked_stat.get_initial_value() != 0:
                if cls.linked_stat.initial_value_range is None:
                    logger.error('{} has a linked stat {} that has initial tuning but Use Stat Value On Init is not checked.', cls,
                      (cls.linked_stat), owner='rmccord')
...
class StateComponent(Component, component_name=STATE_COMPONENT, persistence_key=protocols.PersistenceMaster.PersistableData.StateComponent, persistence_priority=ComponentPriority.PRIORITY_STATE, allow_dynamic=True):
    ...
    def apply_delinquent_states(self, utility=None, from_load=False):
        if self._delinquency_state_changes is None:
            ...
            return self.owner.is_on_active_lot or False
    ...
    def _persist_accross_gallery(self, state):
        if self.owner.is_downloaded:
            if not state.persists_across_gallery:
                return True
        return False
    ...
    def component_reset(self, reset_reason):
        if reset_reason == ResetReason.BEING_DESTROYED:
            ...
            return self.owner.valid_for_distribution or None
    ...
    def pre_parent_change(self, parent):
        if self.enter_carry_state is None and parent is not None and parent.is_sim:
            ...
            if not (self.exit_carry_state is None):
                for value in self.values():
    ...
    def _check_allowances(self, new_value, from_load=False):
        if self.owner.manager is None:
            ...
            if not new_value.allowances.allow_in_carry:
                if owner_parent is not None:
                    if owner_parent.is_sim:
                        ...
                        return False
            if not new_value.allowances.allow_out_of_carry:
            ...
            if not new_value.allowances.allow_outside:
                if is_outside:
                    if is_outside is not None:
                        ...
                        return False
            if not new_value.allowances.allow_inside:
                if not is_outside:
                    if is_outside is not None:
                        ...
                        return False
            is_on_natural_ground = from_load or self.owner.is_on_natural_ground()
            if is_on_natural_ground is None:
                return True
            if not new_value.allowances.allow_on_natural_ground:
                if is_on_natural_ground:
                    if is_on_natural_ground is not None:
                        ...
                        return False
        elif not new_value.allowances.allow_off_natural_ground:
            if not is_on_natural_ground:
                if is_on_natural_ground is not None:
    ...
    def set_state(self, state, new_value, from_stat=False, from_init=False, from_load=False, immediate=False, force_update=False, from_sync=False, **kwargs):
        if not state.sync_tags or from_sync or from_init or from_load:
            ...
            if synced_objects:
                ...
                return self._check_allowances(new_value, from_load=from_load) or None
                    ...
                    if not value_force_added:
                        ...
                        if new_value != old_value or value_force_added:
                            ...
                            if current_zone_id is not None:
                                services.get_event_manager().process_events_for_household((test_events.TestEvent.ObjectStateChange), household=(services.owning_household_of_active_lot()),
                                  custom_keys=(
                                 new_value,))
            logger.debug('State change: {} -> {} ({})', old_value, new_value, 'from_init' if from_init else 'from_stat' if from_stat else 'normal')
            self._states[state] = new_value
            if not from_load:
                if new_value.send_telemetry_on_state_change:
                    current_zone = services.current_zone()
            ...
            if state.sync_tags and not from_sync or from_init:
                if not from_load:
                    synced_objects = services.object_manager().get_objects_matching_tags((state.sync_tags), match_any=True).difference([self.owner])
                    if synced_objects:
    ...
    def disable_timed_state_trigger(self, timed_state, timed_state_change):
        if timed_state is None:
            if timed_state_change in self._active_dynamic_timed_state_triggers:
                ...
                return
            logger.error('Dynamic timed state change {} finished its alarm but was not set as active for object {}', timed_state_change,
    ...
    def _apply_client_state(self, state, value, immediate=False, post_load_distributable_only=False, **kwargs):
        ...
        if state.overridden_by is not None:
            if self.has_state(state.overridden_by):
    ...
    def _save_state_data(self):
        ...
        return (
         states_data, states_before_delinquency_data)
...
class TunableStateTriggerTestVariant(TunableVariant):
    ...
    def __init__(self, description='A single tunable test.', **kwargs):
        (super().__init__)(object_criteria=objects.object_tests.ObjectCriteriaTest.TunableFactory(locked_args={'tooltip': None}), 
         ...
         next_festival=NextFestivalTest.TunableFactory(locked_args={'tooltip': None}), 
...
class _StateTriggerLootActions(TunableVariant):
    ...
    def __init__(self, *args, **kwargs):
        (super().__init__)(args, lock_door=LockDoor.TunableFactory(lock_data_options={'requires_sim_subject': False},
  locked_args={'advertise':False, 
 'chance':SuccessChance.ONE, 
 'subject':ParticipantType.Object, 
 'subject_filter_tests':None}), 
         unlock_door=UnlockDoor.TunableFactory(locked_args={'advertise':False, 
 'chance':SuccessChance.ONE, 
 'subject':ParticipantType.Object, 
 'subject_filter_tests':None}), **kwargs)
...
class StateTrigger(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def _verify_tunable_callback(instance_class, tunable_name, source, value):
        if value.set_random_state is None:
            if value.set_states is None:
                if not value.statistic_operations:
                    if value.set_on_children is None:
                        if not value.loot_actions:
                            logger.error('Object {} has trigger state values at states {} that does nothing.', instance_class, (value.at_states),
                              owner='trevor')
    ...
    FACTORY_TUNABLES = {'set_states':OptionalTunable(description='\n            If enabled it will trigger the states tuned on the list.\n            Either this list or set random state needs to be tuned on a state\n            trigger.\n            ',
     ...
     'set_on_children':OptionalTunable(description='\n            If enabled, will apply states and/or stats to children of this\n            object.\n            ',
      ...
      'subject':ParticipantType.Object},
       include_relationship_ops=False))),
       enabled_name='set_on_children',
       disabled_name='leave_children_alone'), 
     'trigger_operation':TunableEnumEntry(description='\n            The operation to apply on the at_states to decide if we can trigger\n            the at_state. \n            AND:  trigger the new state only if the object is in all the listed \n                  states at the same time. \n            OR:   trigger the new state if the object is in any of the listed \n                  states. \n            NONE: trigger the new state only if the object is in none of the \n                  listed states.\n            ',
       tunable_type=StateTriggerOperation,
       default=StateTriggerOperation.AND), 
     'trigger_chance':OptionalTunable(TunableRange(description='\n                The chance to trigger the target state when we reach the at_state.',
       tunable_type=float,
       default=100,
       minimum=0,
       maximum=100)), 
    ...
    def is_trigger_state_valid(self, owner, at_state, state_to_trigger):
        ...
        if owner.state_component is None:
            ...
            return False
        if owner.state_component.state_value_active(state_to_trigger):
            return False
        return True
    ...
    def _check_triggerable(self, owner, at_state):
        if not self._check_chance():
            ...
            return self._check_prohibited_states(owner) or False
        ...
        if self.trigger_operation == StateTriggerOperation.NONE:
    ...
    def _check_and(self, owner, at_state):
        ...
        for state_value in self.at_states:
            if state_value is None:
                return False
                return owner.state_component.state_value_active(state_value) or False
...
class TunableStateComponent(TunableFactory):
        ...
        def _verify_state_value(state_value):
            if state_value is not None:
                if state_value.state is None:
                    logger.error("State value '{}' triggered in the timed state triggers of Object {} has no tuned state.", state_value,
                      instance_class, owner='shipark')
                if state_value.state not in [object_state.default_value.state for object_state in states]:
                    logger.error("Object {} triggers the state '{}' in its timed state triggers but isn't tuned in the states list of the state component.", instance_class,
                      (state_value.state), owner='shipark')
            ...
            for trigger_item in timed_trigger.ops:
                if trigger_item.states_to_trigger is not None:
                    ...
                    if trigger_time <= prev_trigger_time:
                        logger.error("Object '{}' has a timed state trigger entry which is tuned to occur at time:({}) which is sooner or equal to the previous entry's time:({}).", instance_class,
                          trigger_time, prev_trigger_time, owner='bnguyen')
                    else:
                        if trigger_time <= prev_trigger_time_potential:
                            logger.error("Object '{}' has a timed state trigger entry which is tuned to occur at time:({}) which is sooner or equal to the previous entry's max potential time:({}) when accounting for its random time offset.", instance_class,
                              trigger_time, prev_trigger_time_potential, owner='bnguyen')
  ...
  tunable_type=(household_utilities.utility_types.Utilities),
  default=None),
  ...
  value_name='overlap data')), 
         description=description, 
         verify_tunable_callback=self._verify_tunable_callback, **kwargs)
...
class TunableStateChange(TunableFactory):
    ...
    TunableAtEnd = TunableFactory.create_auto_factory(_state_change_at_end, 'TunableAtEnd', description='Change the state value at the end of the sequence.',
      criticality=TunableEnumEntry(CleanupType, (CleanupType.OnCancel), description='The criticality of making the state change.'))
    TunableOnXevt = TunableFactory.create_auto_factory(_state_change_at_end, 'TunableOnXevt', description='\n        Set the new state value in sync with an animation event with a\n        particular id. In the case no matching event occurs in the animation,\n        the value will still be set at the end of the sequence.\n        ',
      criticality=TunableEnumEntry(CleanupType, (CleanupType.OnCancel), description='The criticality of making the state change.'),
      xevt_id=Tunable(int, 100, description="An xevt on which to change the state's value."))
    ...
    class ValueFromTestList(HasTunableSingletonFactory):
        ...
        def FACTORY_TUNABLES(cls):
            ...
            return {'new_values':TunableList(TunableTuple(test=TunableTestVariant(test_locked_args={'tooltip': None}),
               value=(TunableStateValueReference()))), 
             'fallback_value':OptionalTunable(TunableStateValueReference())}
    ...
    def _factory(interaction, state_change_target, timing, new_value, force_update, **kwargs):
        ...
        return timing(new_value=actual_new_value, targets=actual_state_change_targets, 
         animation_context=interaction.animation_context, 
    ...
    def __init__(self, description='Change the value of a state on a participant of an interaction.', **kwargs):
        (super().__init__)(state_change_target=TunableEnumEntry(ParticipantType, (ParticipantType.Object), description='Who or what to change the state on.'), timing=TunableVariant(description="When to change the state's value.", immediately=(TunableStateChange.TunableAtBeginning()),
  at_end=(TunableStateChange.TunableAtEnd()),
  on_xevt=(TunableStateChange.TunableOnXevt()),
  default='at_end'), 
         new_value=TunableVariant(description='A new value to set.', single_value=(TunableStateChange.TunableSingleValue()),
  value_from_test_list=(TunableStateChange.ValueFromTestList.TunableFactory()),
  value_from_test_set_list=(TunableStateChange.ValueFromTestSetList.TunableFactory()),
  default='single_value'), 
         force_update=Tunable(description="If checked, force update the state's value.", tunable_type=bool,
  default=False), 
         description=description, **kwargs)
...
class TunableTransienceChange(TunableFactory):
    ...
    def __init__(self, description="Change the transience on the interaction's target.", **kwargs):
        (super().__init__)(who=TunableEnumEntry(ParticipantType, (ParticipantType.Object), description='Who or what to apply this test to'), new_value_beginning=TunableVariant(locked_args={'no_change':None,  'make_transient':True,  'make_permanent':False}, default='no_change',
  description='A value to set transience to at the beginning (may be None)'), 
         new_value_ending=TunableVariant(locked_args={'no_change':None,  'make_transient':True,  'make_permanent':False}, default='no_change',
  description='A value to set transience to at the beginning (may be None)'), 
         xevt_id=OptionalTunable(Tunable(int, 100, description="An xevt on which to change the state's value to new_value_ending")), 
         criticality=TunableEnumEntry(CleanupType, (CleanupType.NotCritical), description='The criticality of making these state changes.'), 
         description=description, **kwargs)
...
class TimedStateChange:
    ...
    def _try_schedule_next_trigger_alarm(self, reschedule_current_trigger=False):
        if not reschedule_current_trigger:
            self._trigger_index += 1
        elif self._trigger_index >= len(self._timed_triggers):
...
class TunableStateValueVariant(TunableVariant):
    ...
    def __init__(self, **kwargs):
        (super().__init__)(single_value=TunableStateChange.TunableSingleValue(), 
         value_from_test_list=TunableStateChange.ValueFromTestList.TunableFactory(), 
         value_from_test_set_list=TunableStateChange.ValueFromTestSetList.TunableFactory(), 
         default='single_value', **kwargs)
