...
# Size of source mod 2**32: 52609 bytes
...
class CarryTargetInteraction(ProxyInteraction):
    ...
    def get_participants(cls, inst, participant_type, *args, **kwargs):
        ...
        if participant_type == ParticipantType.CarriedSim:
            if inst_or_cls._carry_target_ref is not None:
...
class CarryableComponent(Component, HasTunableFactory, AutoFactoryInit, component_name=objects.components.types.CARRYABLE_COMPONENT):
    ...
    @componentmethod
    def get_carry_transition_constraint(self, sim, position, routing_surface, cost=0, mobile=True, los_reference_point=DEFAULT):
    ...
    @componentmethod
    def get_pick_up_constraint(self, sim, position, routing_surface, los_reference_point=DEFAULT):
        if self.constraint_pick_up is None:
            return
        final_constraint = Anywhere()
        for constraint in self.constraint_pick_up:
            constraint = self._get_adjusted_circle_constraint(sim, constraint)
            constraint = constraint.create_constraint(sim, target=(self.owner), target_position=position,
              routing_surface=routing_surface,
              los_reference_point=los_reference_point)
    ...
    def get_put_down_aop(self, interaction, context, alternative_multiplier=1, own_inventory_multiplier=1, object_inventory_multiplier=DEFAULT, in_slot_multiplier=DEFAULT, on_floor_multiplier=1, visibility_override=None, display_name_override=None, additional_post_run_autonomy_commodities=None, add_putdown_liability=False, **kwargs):
        ...
        if not put_down_strategy.affordances:
            ...
            if self.owner.is_sim:
                self._attempted_alternative_putdown = True
                scored_aops = []
                for scored_aop in (self._gen_affordance_score_and_aops)(interaction, **, **kwargs):
            ...
            liabilities = ()
        aop = AffordanceObjectPair(affordance, self.owner, affordance, None, **, **kwargs)
 ...
 None, **, **kwargs)
    ...
    def _get_terrain_transform(self, interaction):
                        ...
                        if carry_cancellation_target_ref is not None:
                            ...
                            if put_down_strategy.putdown_near_carry_cancellation_target:
                                ...
                                if carry_cancellation_target is not None:
                                    if carry_cancellation_target.item_location == ItemLocation.ON_LOT or lot.is_position_on_lot(carry_cancellation_target.position, 0):
        ...
        fgl_context = fgl_context_fn(starting_location, (self.owner), max_steps=(FGLTuning.MAX_PUTDOWN_STEPS),
          max_distance=(FGLTuning.MAX_PUTDOWN_DISTANCE),
