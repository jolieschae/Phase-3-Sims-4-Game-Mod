...
# Size of source mod 2**32: 169383 bytes
        ...
        for household in households:
            ...
            if not self.required_count.compare(count):
                return TestResultNumeric(False, '{} Household Inventory does not have required number of objects in it',
                  ...
                  tooltip=(self.tooltip),
                  current_value=count,
                  goal_value=required_count_value,
                  is_money=False)
                    ...
                    if self.test_hidden != self.TestHiddenInventories.ANY:
                        ...
                        if is_hidden != hidden_only:
                            ...
                            if content_check_type == self.ITEM_DEFINITION_TEST:
                                ...
                                if content_check_type == self.ITEM_STATE_TEST:
                        ...
                        if count > required_count_value:
                            if operator == Operator.GREATER or operator == Operator.GREATER_OR_EQUAL or operator == Operator.NOTEQUAL:
                                return TestResultNumeric(True, current_value=count,
                                  goal_value=required_count_value,
                                  is_money=False)
                                if not operator == Operator.EQUAL:
                                    if not operator == Operator.LESS_OR_EQUAL:
                                        if operator == Operator.LESS:
                                            pass
                            return TestResultNumeric(False, 'Inventory {} has more objects than required',
                              inv,
                              tooltip=(self.tooltip),
                              current_value=count,
                              goal_value=required_count_value,
        ...
        for target in targets:
            ...
            if self.negate:
                ...
                if requirements_result:
                    ...
                    for specified_parent in parent_objects:
                        if parent is specified_parent:
                            return TestResult(False, 'Parent test fail because object {} is parented to {}', target,
                ...
                if parent is None:
                    ...
                    requirements_result = _test_parent_requirements()
                    return requirements_result or requirements_result
                if parent_objects is not None:
                    if parent.is_sim:
                        parent = parent.sim_info
                    for specified_parent in parent_objects:
                        if parent is not specified_parent:
                            return TestResult(False, "Parent test fail because object {} is not parented to the specified parent {}. It's parented to {}.", target,
                              specified_parent, parent, tooltip=(self.tooltip))
            ...
            for obj_or_sim_info in target_list:
                if obj_or_sim_info.is_sim:
                    ...
                    if obj is None:
                        ...
                        obj = obj_or_sim_info
                    return sim.can_see(obj) or TestResult(False, ("{} can't see {}.".format(sim, obj)), tooltip=(self.tooltip))
            ...
            if not participant.is_sim:
                ...
                if self.current_game_rule is not None:
                    if game.current_game is None:
                        return self.current_game_rule.negate or TestResult(False, 'GameTest: Cannot check current game rule because no game has been started.', tooltip=(self.tooltip))
                    else:
                        if (game.current_game.guid64 == self.current_game_rule.rule.guid64) == self.current_game_rule.negate:
                            return TestResult(False, 'GameTest: Current game ({}) {} tuned game to look for ({}) and we are {} default test behavior.', (game.current_game.guid64),
                              ('matches' if self.current_game_rule.negate else "doesn't match"),
                              (self.current_game_rule.rule.guid64),
                              ('negating' if self.current_game_rule.negate else 'using'),
                              tooltip=(self.tooltip))
