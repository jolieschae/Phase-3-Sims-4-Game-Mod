...
# Size of source mod 2**32: 253563 bytes
...
class CareerBase(CoworkerMixin, CareerKnowledgeMixin, sims.sim_spawner_service.ISimSpawnerServiceCustomer):
        ...
        if self.is_work_time:
            if self.taking_day_off:
                self._populate_work_state_time_off_msg(msg, (self.taking_day_off_reason),
                  enable_go_to_work=(not self._sim_info.is_in_travel_group(group_type=(FileSerialization_pb2.TravelGroupData.GROUPTYPE_VACATION))))
            else:
            ...
            if self.career_panel_type == CareerPanelType.UNIVERSITY_COURSE:
                degree_tracker = self._sim_info.is_in_travel_group(group_type=(FileSerialization_pb2.TravelGroupData.GROUPTYPE_VACATION)) or self._sim_info.degree_tracker
                ...
                if self.career_panel_type == CareerPanelType.FREELANCE_CAREER:
                    ...
                    if self.requested_day_off:
                        if self.requested_day_off_reason != career_ops.CareerTimeOffReason.WORK_FROM_HOME:
                            ...
                            if self._sim_info.is_in_travel_group(group_type=(FileSerialization_pb2.TravelGroupData.GROUPTYPE_VACATION)):
                                if self.pto > 0:
    ...
    def join_career(self, career_history=None, career_track=None, user_level_override=None, career_level_override=None, give_skipped_rewards=True, defer_rewards=False, schedule_shift_override=CareerShiftType.ALL_DAY, show_join_msg=True, disallowed_reward_types=(), force_rewards_to_sim_info_inventory=False, defer_first_assignment=False, schedule_init_only=False, allow_outfit_generation=True, show_icon_override_picker=True):
        ...
        self.career_start(schedule_init_only=schedule_init_only, allow_outfit_generation=allow_outfit_generation,
          test_event_origin=(TestEventCareersOrigin.JOIN_CAREER))
    ...
    def career_start(self, is_load=False, is_level_change=False, schedule_init_only=False, allow_outfit_generation=True, test_event_origin=TestEventCareersOrigin.UNSPECIFIED):
        ...
        if is_load:
            ...
            services.get_event_manager().process_event((test_events.TestEvent.CareerEvent), sim_info=(self._sim_info),
              ...
              track=(self.current_track_tuning),
              level=(self.level),
              test_event_origin=test_event_origin)
    ...
    def _get_common_performance_change(self):
        ...
        return (
         gain, loss, tested_performance_multipliers)
    ...
    def _apply_tested_performance_multipliers(self, performance_multipliers, delta):
        for performance_multiplier in performance_multipliers:
            if not delta > 0 or performance_multiplier.apply_direction == StatisticChangeDirection.INCREASE or delta < 0 and performance_multiplier.apply_direction == StatisticChangeDirection.DECREASE or performance_multiplier.apply_direction == StatisticChangeDirection.BOTH:
    ...
    def _start_work_callback(self, scheduler, alarm_data, extra_data):
        if self._at_work:
            ...
            if self._sim_info.is_in_travel_group(group_type=(FileSerialization_pb2.TravelGroupData.GROUPTYPE_VACATION)):
                ...
                def get_situation_jobs--- This code section failed: ---

 L.2301         0  LOAD_GLOBAL              services
                2  LOAD_METHOD              get_zone_situation_manager
                4  CALL_METHOD_0         0  '0 positional arguments'
                6  STORE_FAST               'situation_manager'

 L.2302         8  LOAD_CLOSURE             'sim'
               10  BUILD_TUPLE_1         1 
               12  LOAD_SETCOMP             '<code_object <setcomp>>'
               14  LOAD_STR                 'CareerBase._start_work_callback.<locals>.get_situation_jobs.<locals>.<setcomp>'
               16  MAKE_FUNCTION_8          'closure'
               18  LOAD_FAST                'situation_manager'
               20  LOAD_METHOD              get_situations_sim_is_in
               22  LOAD_DEREF               'sim'
...
Parse error at or near `LOAD_SETCOMP' instruction at offset 12
                ...
                if not self.taking_day_off:
                    if not self._sim_info.is_in_travel_group(group_type=(FileSerialization_pb2.TravelGroupData.GROUPTYPE_VACATION)):
                        sim = self._get_sim()
                        situation_jobs = None
                        if self._sim_info.is_npc:
                            if sim is not None:
    ...
    def _prune_stale_career_event_cooldowns--- This code section failed: ---

 L.2802         0  LOAD_FAST                'self'
                2  LOAD_ATTR                days_worked_statistic
                4  LOAD_METHOD              get_value
                6  CALL_METHOD_0         0  '0 positional arguments'
                8  STORE_DEREF              'current_day'

 L.2803        10  LOAD_CLOSURE             'current_day'
               12  BUILD_TUPLE_1         1 
               14  LOAD_DICTCOMP            '<code_object <dictcomp>>'
               16  LOAD_STR                 'CareerBase._prune_stale_career_event_cooldowns.<locals>.<dictcomp>'
               18  MAKE_FUNCTION_8          'closure'
               20  LOAD_FAST                'self'
               22  LOAD_ATTR                _career_event_cooldown_map
               24  LOAD_METHOD              items
               26  CALL_METHOD_0         0  '0 positional arguments'
               28  GET_ITER         
               30  CALL_FUNCTION_1       1  '1 positional argument'
               32  LOAD_FAST                'self'
               34  STORE_ATTR               _career_event_cooldown_map

Parse error at or near `LOAD_DICTCOMP' instruction at offset 14
        ...
        if self.sim_info.is_npc:
            ...
            if tutorial_service is not None:
                if tutorial_service.is_tutorial_running():
                    return False
        elif self.block_promotion_tests is not None:
            if len(self.block_promotion_tests) != 0:
                resolver = SingleSimResolver(self.sim_info)
    ...
    def lay_off(self, post_lay_off_msg=False):
        ...
        return career_service is None or career_service.career_lay_off_enabled or None
        resolver = SingleSimResolver(self._sim_info)
        ...
        self._sim_info.career_tracker.remove_career((self.guid64), post_quit_msg=False,
          test_event_origin=(TestEventCareersOrigin.LAY_OFF))

    def quit_career(self, post_quit_msg=True, update_ui=True, test_event_origin=TestEventCareersOrigin.QUIT_CAREER):
    ...
    def _handle_promotion(self, previous_salary, previous_highest_level):
        ...
        services.get_event_manager().process_event((test_events.TestEvent.CareerPromoted), sim_info=(self._sim_info),
          career=self,
          level=(self.level),
          track=(self.current_track_tuning),
          test_event_origin=(TestEventCareersOrigin.PROMOTION))
        current_gig = self.get_current_gig()
        if current_gig is not None:
    ...
    def _fire(self):
        self._sim_info.career_tracker.remove_career((self.guid64), post_quit_msg=False,
          test_event_origin=(TestEventCareersOrigin.FIRED))
    ...
    def evaluate_career_performance(self, money_earned, pto_earned, gig=None):
        ...
        current_performance = self.work_performance
        resolver = SingleSimResolver(self._sim_info)
        if self.can_be_fired or self.can_change_level(demote=True):
    ...
    def populate_set_career_op_with_data_for_played_sims(self, career_op, gig=None):
        ...
        if self.career_category == CareerCategory.Work:
