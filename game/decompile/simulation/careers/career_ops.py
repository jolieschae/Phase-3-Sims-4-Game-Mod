    ...
    def _apply_to_career(self, sim_info, career, resolver):
        for op in self.operations:
            if op.operation_type == CareerLootOp.OP_PERFORMANCE:
                ...
                if op.operation_type == CareerLootOp.OP_LAY_OFF:
                    career.lay_off(post_lay_off_msg=(op.show_notification))
            if op.operation_type == CareerLootOp.OP_JOIN:
                if career is None:
                    career_reference = self.career.reference(sim_info)
                    if isinstance(op.level_to_join, int):
                        sim_info.career_tracker.add_career(career_reference, show_confirmation_dialog=(op.show_confirmation_dialog),
                          user_level_override=(op.level_to_join))
                    else:
                        sim_info.career_tracker.add_career(career_reference, show_confirmation_dialog=(op.show_confirmation_dialog),
                          career_level_override=(op.level_to_join))
