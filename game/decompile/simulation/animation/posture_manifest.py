...
# Size of source mod 2**32: 51589 bytes
...
class PostureManifestEntry(_PostureManifestEntry, InternMixin):
    ...
    def apply_actor_map(self, actor_map):
        ...
        return self.__class__(actor, (self.specific), (self.family), (self.level), left,
          right, back, surface, (self.provides), target_object_filter=(self.target_object_filter))
    ...
    def get_entries_with_override(self, override_value: PostureManifestOverrideValue):
        ...
        if (right is None or self.right) == MATCH_ANY:
            if right != MATCH_NONE:
                right = self.right
        if (back is None or self.back) == MATCH_ANY:
            if back != MATCH_NONE:
                back = self.back
        return [
         self._with_overrides(surface=surface, left=left, right=right, back=back)]
    ...
    def carry_target(self):
        ...
        if self.right not in _NOT_SPECIFIC_ACTOR:
            return self.right
        if self.back not in _NOT_SPECIFIC_ACTOR:
            return self.back
