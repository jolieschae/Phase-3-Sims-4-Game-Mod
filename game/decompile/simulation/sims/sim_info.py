...
# Size of source mod 2**32: 221626 bytes
import singletons
...
class SimInfo(SimInfoWithOccultTracker, SimInfoCreationSource.SimInfoCreationSourceMixin, AgingMixin, PregnancyClientMixin, SimInfoFavoriteMixin, ComponentContainer, HasStatisticComponent):
    ...
    SIM_INFO_TRACKERS = OrderedDict((
     ...
     (
      '_developmental_milestone_tracker', DevelopmentalMilestoneTracker)))
 ...
 None, away_action=away_action, 
    ...
    def _load_sim_info(self, sim_proto, skip_load, is_clone=False):
        ...
        if False and not self.full_name_key != 0:
            if self.first_name_key != 0 or self.last_name_key != 0:
            ...
            if sim_proto.needs_age_progress_randomized:
                ...
                if hasattr(sim_proto.gameplay_data, 'build_buy_unlock_list'):
                    ...
                    if not (self._primary_aspiration is None or self._primary_aspiration.is_available()):
                        if self.is_human:
            ...
            if sim_attribute_data:
                try:
                    try:
                        ...
                        if self._lunar_effect_tracker is not None:
                            if not skip_load:
            ...
            if (skip_load or sim_proto.gameplay_data.HasField)('location'):
            ...
            if sim_proto.gameplay_data.HasField('gameplay_options'):
                ...
                if self.get_gameplay_option(SimInfoGameplayOptions.FORCE_CURRENT_ALLOW_FAME_SETTING):
                    self.allow_fame = self.get_gameplay_option(SimInfoGameplayOptions.ALLOW_FAME) or False
        else:
            if self.get_gameplay_option(SimInfoGameplayOptions.FREEZE_FAME):
    ...
    def _get_time_to_go_home(self):
        if self.is_toddler_or_younger:
            travel_group = self.travel_group
            if travel_group is not None:
                if travel_group.group_type == serialization.TravelGroupData.GROUPTYPE_STAYOVER:
                    if self._zone_id == travel_group.zone_id:
    ...
    def on_all_households_and_sim_infos_loaded(self):
        ...
        if not self.sim_template_id or self.premade_sim_fixup_completed:
