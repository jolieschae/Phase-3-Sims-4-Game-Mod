...
# Size of source mod 2**32: 41523 bytes
...
class PregnancyTracker(SimInfoTracker):
    ...
    def start_pregnancy(self, parent_a, parent_b, pregnancy_origin=PregnancyOrigin.DEFAULT, single_sim_is_allowed=False):
        if not self.is_pregnancy_supported:
            ...
            if self.is_pregnant:
                return
            if not parent_a != parent_b or parent_a.incest_prevention_test(parent_b):
                if parent_a == parent_b:
                    return single_sim_is_allowed or None
    ...
    def create_sim_info(self, offspring_data):
        ...
        sim_info_list, _ = SimSpawner.create_sim_infos((sim_creator,), household=household,
          ...
          household_change_origin=(HouseholdChangeOrigin.BIRTH))
