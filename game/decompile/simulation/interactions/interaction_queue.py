...
# Size of source mod 2**32: 80633 bytes
...
class BucketBase:
    ...
    def get_next_unblocked_interaction_cancel_incompatible(self, blocked_sims_callback=None):
        ...
        for interaction in self:
            ...
            if interaction.is_finishing:
                continue
            if interaction.is_waiting_pickup_putdown:
                continue
...
class InteractionBucket(BucketList):
 ...
 L. 315         0  LOAD_FAST                'interaction'
                2  LOAD_ATTR                is_super
                4  POP_JUMP_IF_TRUE     28  'to 28'

 L. 316         6  LOAD_GLOBAL              len
                8  LOAD_FAST                'self'
 ...
 L. 317        20  LOAD_FAST                'interaction'
 ...
 L. 323        42  SETUP_LOOP          134  'to 134'
 ...
 L. 324        62  LOAD_FAST                'queued_interaction'
 ...
 L. 325        82  LOAD_FAST                'queued_interaction'
 ...
 L. 331       116  BREAK_LOOP       
              118  JUMP_BACK            54  'to 54'
              120  POP_BLOCK        

 L. 333       122  LOAD_FAST                'self'
 ...
 L. 334       134  LOAD_GLOBAL              TestResult
              136  LOAD_ATTR                TRUE
              138  RETURN_VALUE     
               -1  RETURN_LAST      
        ...
        for interaction in reversed(list(self)):
            ...
            if highest_priority_interaction is not None:
                if not interaction.is_related_to(highest_priority_interaction):
                    if can_displace(highest_priority_interaction, interaction, allow_clobbering=allow_clobbering, use_max_priority=True):
