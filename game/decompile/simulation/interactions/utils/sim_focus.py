...
# Size of source mod 2**32: 14216 bytes
...
class TunableFocusElement(TunableFactory):
    ...
    @staticmethod
    def factory(interaction, subject, layer, score, focus_bone_override=None, disable_focus=False, focus_layer_override=None, sequence=()):
        sim = interaction.sim
        if disable_focus:
            sequence = without_sim_focus(sim, sim, sequence)
            return sequence
        elif focus_layer_override is not None:
    ...
    def __init__(self, description="Configure focus on one or more of an interaction's participants.", **kwargs):
        (super().__init__)(subject=TunableEnumFlags(ParticipantType, (ParticipantType.Object), description='Who or what to focus on.'), layer=Tunable(int, None, description='Layer override: Ambient=0, SuperInteraction=3, Interaction=5.'), 
         ...
         disable_focus=Tunable(bool, False, description='If True, we disable the focus for the interaction participants.'), 
...
class TestedTunableFocusElement(TunableFactory):
    ...
    @staticmethod
    def factory(interaction, tested_focus_list, sequence=()):
    ...
    def __init__(self, description='The first tunable focus whose tests pass will be applied.', **kwargs):
