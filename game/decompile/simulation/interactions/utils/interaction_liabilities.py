...
# Size of source mod 2**32: 21285 bytes
                ...
                if (posture.is_vehicle or posture).ownable:
                    if not self._posture.owning_interactions or len(self._posture.owning_interactions) == 1 and self._interaction_cancel_replacement_ref() in self._posture.owning_interactions:
                        if not posture.unconstrained:
                            sim.is_being_destroyed or sim.schedule_reset_asap(source=(self._posture.target), cause=("CancelAOPLiability released without changing the Sim's posture away from {}".format(self._posture)))
                        else:
