...
class DistancePlacementMixin:
    ...
    def _update_water_depth_requirements(self, sim_a, sim_b, interval, **kwargs):
        ...
        if not water_depth_info is None or min_water_depth is not None or max_water_depth is not None:
    ...
    def _entered_pipeline(self):
        if self.carry_target is not None:
            ...
            raytest_info = RaytestInfo(raytest_radius=(self.raytest_radius), raytest_start_offset=(self.raytest_offset.lower_bound),
              raytest_end_offset=(self.raytest_offset.upper_bound))
            fgl_kwargs = {'raytest_info':raytest_info,  'ignored_object_ids':{sim.id for sim in self.required_sims()}, 
             'positioning_type':JigPositioning.RelativeToSimA}
