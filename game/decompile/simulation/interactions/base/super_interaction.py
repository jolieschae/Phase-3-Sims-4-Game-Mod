...
# Size of source mod 2**32: 305721 bytes
        ...
        if cls._can_rally(context):
            for entry in cls.rallyable:
                ...
                rally_interaction = RallyInteraction.generate(affordance, rally_tag=(entry.tag), rally_level=0, rally_data=(entry.behavior),
                  ...
                  rally_carry_rule=(entry.rally_carry_rule))
...
True # Avoid dead code: False
        ...
        if posture is None:
            ...
            if posture.is_vehicle:
                ...
                if posture.track == PostureTrack.BODY:
                    ...
                    if posture.track == PostureTrack.BACK:
    ...
    def get_cancel_replacement_aops_contexts_postures(self, can_transfer_ownership=True, carry_cancel_override=None):
        ...
        if can_transfer_ownership:
            if carry_target is not None:
                if carry_target.parent is self.sim:
                    head_interaction = self.is_putdown or self.sim.queue.get_head()
                    if head_interaction is not None:
                        if head_interaction.is_super:
                            if not head_interaction is not self or head_interaction.carry_target is carry_target or head_interaction.target is carry_target:
        ...
        for posture in reversed(self.sim.posture_state.aspects):
            if new_carry_owner is not None:
                if new_carry_owner.can_acquire_posture_ownership and posture.target is carry_target:
            ...
            if posture.last_owning_interaction(self) and proxy_posture_owner is None:
        ...
        try:
            try:
                ...
                if self._interrupt_active_work(finishing_type=finishing_type, cancel_reason_msg=cancel_reason_msg):
                    if not (self._try_exit_via_cancel_aop)(carry_cancel_override=carry_cancel_override, cancellation_originator=cancellation_originator, **kwargs):
                        self._active = False
                        ...
                        if self.sim is not None:
                            ...
                            for posture in self.sim.posture_state.aspects:
                                if self in posture.owning_interactions and len(posture.owning_interactions) == 1 and posture.source_interaction is not None and posture.source_interaction is not self:
 ...
 L.5618        30  BUILD_MAP_0           0 
               32  RETURN_VALUE     
             34_0  COME_FROM            28  '28'

 L.5620        34  LOAD_FAST                'skill_stat'
               36  LOAD_DEREF               'self'
 ...
 L.5621        48  LOAD_FAST                'tested_result'
               50  LOAD_CONST               None
               52  COMPARE_OP               is
               54  POP_JUMP_IF_FALSE    60  'to 60'

 L.5622        56  BUILD_MAP_0           0 
               58  RETURN_VALUE     
             60_0  COME_FROM            54  '54'

 L.5624        60  LOAD_FAST                'skill_sim'
 ...
 L.5625        70  LOAD_FAST                'animation_stat'
               72  LOAD_CONST               None
               74  COMPARE_OP               is-not
               76  POP_JUMP_IF_FALSE   120  'to 120'

 L.5626        78  LOAD_FAST                'animation_stat'
 ...
 L.5627        90  LOAD_FAST                'asm_param_name'
               92  LOAD_CONST               None
 ...
 L.5628       106  LOAD_FAST                'asm_param_name'
 ...
 L.5633       152  LOAD_FAST                'asm_param_name'
              154  LOAD_CONST               None
 ...
 L.5634       168  LOAD_FAST                'asm_param_name'
...
Parse error at or near `RETURN_VALUE' instruction at offset 182
 ...
 L.5656        30  LOAD_CONST               None
               32  RETURN_VALUE     
             34_0  COME_FROM            28  '28'

 L.5658        34  LOAD_FAST                'skill_stat'
               36  LOAD_DEREF               'self'
               38  LOAD_METHOD              get_resolver
               40  CALL_METHOD_0         0  '0 positional arguments'
               42  LOAD_CONST               ('resolver',)
 ...
 L.5659        48  LOAD_FAST                'tested_result'
               50  LOAD_CONST               None
               52  COMPARE_OP               is
               54  POP_JUMP_IF_FALSE    60  'to 60'

 L.5660        56  LOAD_CONST               None
               58  RETURN_VALUE     
             60_0  COME_FROM            54  '54'

 L.5662        60  LOAD_FAST                'skill_sim'
 ...
 L.5663        74  LOAD_FAST                'animation_stat'
               76  LOAD_CONST               None
               78  COMPARE_OP               is-not
               80  POP_JUMP_IF_FALSE   126  'to 126'

 L.5664        82  LOAD_FAST                'animation_stat'
 ...
 L.5665        94  LOAD_FAST                'asm_param_name'
               96  LOAD_CONST               None
...
Parse error at or near `COME_FROM' instruction at offset 126_1
 ...
 L.5686         0  LOAD_FAST                'walkstyle_sim'
                2  LOAD_CONST               None
                4  COMPARE_OP               is
                6  POP_JUMP_IF_TRUE     30  'to 30'
 ...
 L.5688        30  LOAD_CONST               None
               32  RETURN_VALUE     
             34_0  COME_FROM            28  '28'

 L.5690        34  LOAD_FAST                'walkstyle'
