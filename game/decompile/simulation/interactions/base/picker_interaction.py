...
# Size of source mod 2**32: 213632 bytes
...
import build_buy, element_utils, enum, event_testing.results, gsi_handlers, services, sims, sims4.resources, tag, telemetry_helper
...
Parse error at or near `COME_FROM' instruction at offset 44_0
    ...
    def _get_valid_sim_choices_gen(cls, inst, target, context, test_function=None, min_required=1, included_override=set(), excluded_override=set(), **kwargs):
        ...
        for sim_info in sim_infos:
            if not sim_info.can_instantiate_sim:
                ...
                if sim_info.sim_id in excluded_override:
                    continue
                else:
    ...
    def _push_continuation(self, sim, tunable_continuation, sim_ids, insert_strategy, picked_zone_set):
        ...
        self.push_tunable_continuation(tunable_continuation, insert_strategy=insert_strategy,
          picked_item_ids=picked_item_set,
          picked_zone_ids=picked_zone_set,
          multi_push=(self.continuations_are_multi_push))
    ...
    def _push_continuations(self, sim_ids, zone_datas=None):
        ...
        if self.picked_continuation:
            ...
            for index, target_sim_id in enumerate(sim_ids):
                ...
                self.push_tunable_continuation(continuation, insert_strategy=insert_strategy,
                  actor=target_sim,
                  picked_zone_ids=picked_zone_set,
                  multi_push=(self.continuations_are_multi_push))
...
class ObjectPickerInteraction(ObjectPickerMixin, PickerSingleChoiceSuperInteraction):
    ...
    INSTANCE_TUNABLES = {'picker_dialog':TunablePickerDialogVariant(description='\n            The object picker dialog.\n            ',
     ...
     'object_name_override':OptionalTunable(description='\n            If enabled, we override object name (row name) from either tooltip field or Loc text\n            ',
       ...
       from_tooltip=(_DescriptionFromTooltip.TunableFactory()),
       from_text=(_DescriptionFromText.TunableFactory()),
       from_gameplay_object_preference_type=(_DescriptionFromGameplayObjectPreferenceType.TunableFactory()),
       default='from_tooltip'),
       tuning_group=GroupNames.PICKERTUNING), 
...
class PurchasePickerWithContinuationInteraction(PurchasePickerInteraction):
    ...
    def _push_purchase_continuation(self, dialog):
        ...
        if purchased_item_ids is not None:
            ...
            self.push_tunable_continuation((self.continuation), purchased_item_ids=purchased_item_ids,
              insert_strategy=(QueueInsertStrategy.LAST))
