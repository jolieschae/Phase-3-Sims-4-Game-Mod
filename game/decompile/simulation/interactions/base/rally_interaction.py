...
# Size of source mod 2**32: 19579 bytes
...
class RallyInteraction(ProxyInteraction):
    ...
    def _do_rally_behavior(self, sim, constraint):
        if sim is self.sim:
            ...
            if affordance is None and static_commodity is None:
                return False
        if static_commodity is None:
            ...
            if affordance is not None:
                ...
                result = (sim.push_super_affordance)(affordance, target, context, from_rally_interaction=self, 
    ...
    def _process_for_carry(self, group_sims_list):
        ...
        if self.sim in self._eligible_carryable_sims:
            if self.context.preferred_carrying_sim is None:
                preferred_carrying_sim = self._get_preferred_carrying_sim()
                if preferred_carrying_sim is not None:
