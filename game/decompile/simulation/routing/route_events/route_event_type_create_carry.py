...
class RouteEventTypeCreateCarry(_RouteEventTypeCarry):
    ...
    def _execute_internal(self, actor):
        ...
        carry_element_helper = CarryElementHelper(interaction=route_interaction, carry_target=created_object,
          owning_affordance=(self.carry_interaction),
          carry_track=(PostureTrack.RIGHT),
          si_target_participant_type=(ParticipantType.CarriedObject),
          sequence=(build_element(_send_arb)),
          sim=actor,
          asm_context=(asm.context),
          priority_override=(self.priority_override),
          si_target_override=created_object)
