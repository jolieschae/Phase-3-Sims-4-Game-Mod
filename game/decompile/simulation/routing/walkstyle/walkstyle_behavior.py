...
# Size of source mod 2**32: 36348 bytes
...
class WalksStyleBehavior(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def _apply_run_walkstyle_to_path(self, actor, path, walkstyle_overrides, time_offset=None):
        ...
        if run_walkstyle is None:
            return
        run_required_total_distance = max((override for override in walkstyle_overrides if override.run_required_total_distance is not None), key=(operator.attrgetter('walkstyle_behavior_priority')),
          default=self).run_required_total_distance
        if path.length() < run_required_total_distance:
            return
        run_required_segment_distance = max((override for override in walkstyle_overrides if override.run_required_segment_distance is not None), key=(operator.attrgetter('walkstyle_behavior_priority')),
    ...
    def _get_property_override(self, actor, property_name):
        ...
        override = max((override for override in overrides if getattr(override, property_name) is not None), key=(operator.attrgetter('walkstyle_behavior_priority')),
          default=self)
