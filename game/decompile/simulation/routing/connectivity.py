...
# Size of source mod 2**32: 28528 bytes
...
class RoutingHandle(Handle):
    ...
    def get_los_reference_point(self, routing_surface, override_multi_surface=None):
        if (override_multi_surface is True or override_multi_surface) is not False:
    ...
    def get_goals(self, max_goals=None, relative_object=None, single_goal_only=False, for_carryable=False, for_source=False, goal_height_limit=None, target_reference_override=None, always_reject_invalid_goals=False, perform_los_check=True, out_result_info=None, check_height_clearance=True):
        ...
        if override_multi_surface is None:
            if relative_object is not None:
                if relative_object.is_sim:
                    if relative_object.routing_surface != self.sim.routing_surface:
                        override_multi_surface = True
        elif (override_multi_surface is True or self.constraint).multi_surface:
            routing_surfaces = for_source or self.constraint.get_all_valid_routing_surfaces(override_multi_surface=override_multi_surface)
        else:
            routing_surfaces = {
        ...
        if self.sim.is_sim:
            if self.sim.posture.multi_sim:
                if self.sim.posture.should_carry_sim_on_exit:
