...
def has_choices(target_id=None, pick_type=PickType.PICK_TERRAIN, x=0.0, y=0.0, z=0.0, lot_id=0, level=0, control=0, alt=0, shift=0, reference_id=0, is_routable=True, _connection=None):
...
def _get_targets_from_pick(sim, pick_target, pick_type, position, level, zone_id, lot_id, is_routable, preferred_objects=()):
    potential_targets = []
    pool_block_id = 0
...
def generate_choices(target_id=None, pick_type=PickType.PICK_TERRAIN, x=0.0, y=0.0, z=0.0, lot_id=0, level=0, control=0, alt=0, shift=0, reference_id=0, referred_object_id=0, preferred_object_id=0, is_routable=True, _connection=None):
    if not alt:
        if control:
            return 0
        if target_id is None:
...
def generate_phone_choices(control=0, alt=0, shift=0, reference_id=0, _connection=None):
    client = services.client_manager().get(_connection)
    sim = _active_sim(client)
    if sim is None:
        return 0
...
def cancel_mixer_interaction(interaction_id, mixer_id, server_ref, context_handle=None, _connection=None):
