...
# Size of source mod 2**32: 37003 bytes
import traceback
...
def archive_FGL(fgl_id, context, result, time_s):
    ...
    footprints = []
    if context.object_footprints is not None:
    ...
    if raytest_info is not None:
        raytest_info_details = [{'name':'raytest_start_offset',  'value':str(raytest_info.raytest_start_offset)},
         {'name':'raytest_end_offset', 
          'value':str(raytest_info.raytest_end_offset)},
         {'name':'raytest_radius', 
          'value':str(raytest_info.raytest_radius)},
         {'name':'raytest_ignore_flags', 
          'value':str(raytest_info.raytest_ignore_flags)},
         {'name':'raytest_start_point_override', 
          'value':str(raytest_info.raytest_start_point_override)}]
    entry['Raytest Info'] = raytest_info_details
    water_depth_details = []
    water_depth_info = context.get_water_depth_info()
    if water_depth_info is not None:
        water_depth_details = [{'name':'max_water_depth',  'value':str(water_depth_info.max_water_depth)},
         {'name':'min_water_depth', 
          'value':str(water_depth_info.min_water_depth)},
         {'name':'max_pond_water_depth', 
          'value':str(water_depth_info.max_pond_water_depth)},
         {'name':'min_pond_water_depth', 
          'value':str(water_depth_info.min_pond_water_depth)}]
    ...
    if offset_info is not None:
        offset_info_details = [{'name':'offset_distance',  'value':str(offset_info.offset_distance)},
         {'name':'offset_restrictions', 
          'value':str(offset_info.offset_restrictions)}]
