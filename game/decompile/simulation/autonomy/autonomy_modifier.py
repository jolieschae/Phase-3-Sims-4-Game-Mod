...
# Size of source mod 2**32: 57578 bytes
import collections
from autonomy.autonomy_modifier_enums import SuperAffordanceSuppression, SuppressionCheckOption
...
class AutonomyModifier(BaseGameEffectModifier):
    ...
    FACTORY_TUNABLES = {'verify_tunable_callback':_verify_tunable_callback,  'description':"\n            An encapsulation of a modification to Sim behavior.  These objects\n            are passed to the autonomy system to affect things like scoring,\n            which SI's are available, etc.\n            ", 
     ...
     'affordance_compatibility':OptionalTunable(description='\n            Tune this to suppress certain autonomous affordances run by a sim when the sim has this modifier.\n            ',
       tunable=TunableAffordanceFilterSnippet()), 
     'autonomy_constraints':TunableList(description='\n            A list of constraints the autonomy interaction must fulfill in order to\n            be considered.\n            ',
       tunable=TunableTuple(constraint=TunableConstraintVariant(description='\n                    A constraint the autonomy interaction must fulfill in order to\n                    be considered.\n                    '),
       target_to_self=Tunable(description='\n                    If enabled, the constraint will be targeting to the sim having this modifier.\n                    If disabled, the constraint will be targeting to the target of the autonomy aop candidate. \n                    ',
       tunable_type=bool,
       default=False))), 
    ...
    def __init__(self, score_multipliers, static_commodity_score_multipliers, relationship_score_multiplier_with_buff_on_target, provided_affordance_compatibility, affordance_compatibility, autonomy_constraints, super_affordance_suppression_mode, suppress_self_affordances, suppress_preroll_autonomy, super_affordance_suppress_on_add, locked_stats, locked_stats_autosatisfy_on_unlock, decay_modifiers, statistic_modifiers, skill_tag_modifiers, commodities_to_add, commodities_value_persist_for_travel, only_scored_stats, only_scored_static_commodities, stat_use_multiplier, relationship_multipliers, object_tags_that_override_off_lot_autonomy, off_lot_autonomy_rule, override_convergence_value, subject, exclusive_si, decay_modifier_by_category, situation_type_social_score_multiplier, transition_from_sit_posture_penalty, supress_outside_objects_if_sun_out, outside_objects_multiplier, distance_to_object_score_modifier=NoneNoneNoneNoneNoneNoneSuperAffordanceSuppression.AUTONOMOUS_ONLYFalseFalseFalseNoneNoneNoneNoneNone()FalseNoneNoneNoneNoneNoneNoneNoneNoneNoneNoneNoneNoneNoneNoneNone, interaction_score_modifier=None, tested_relationship_score_multipliers=None, prevent_skills_from_being_added=False):
        self._provided_affordance_compatibility = provided_affordance_compatibility
        self._affordance_compatibility = affordance_compatibility
        self._autonomy_constraints = autonomy_constraints
    ...
    def affordance_suppressed(self, sim, aop_or_interaction, user_directed=DEFAULT, check_option=SuppressionCheckOption.PROVIDED_AFFORDANCE_ONLY):
        ...
        if not user_directed:
            if self._super_affordance_suppression_mode == SuperAffordanceSuppression.USER_DIRECTED:
                return False
        if not check_option == SuppressionCheckOption.AFFORDANCE_ONLY or self._affordance_compatibility is None or self._affordance_compatibility(affordance):
            return False
        else:
            if check_option == SuppressionCheckOption.PROVIDED_AFFORDANCE_ONLY:
                if self._provided_affordance_compatibility(affordance):
                    return False
            elif self._affordance_compatibility is None or self._affordance_compatibility(affordance):
                if self._provided_affordance_compatibility(affordance):
                    return False
    ...
    @property
    def autonomy_constraints(self):
        return self._autonomy_constraints
