...
# Size of source mod 2**32: 196574 bytes
import math
...
class CareerFilterTerm(InvertibleFilterTerm):
    ...
    def conform_sim_info_to_filter_term(self, created_sim_info, requesting_sim_info=None, **kwargs):
        ...
        for career in careers:
            if requesting_career_tracker is not None:
                if requesting_career_tracker.get_career_by_uid(career.guid64) is None:
                    continue
                else:
                    if self.invert_score:
                ...
                if career.is_valid_career(created_sim_info):
                    ...
                    if self.career_level is not None:
                        career_tracker.add_career(career, user_level_override=(self.career_level.user_level_override()),
                          career_level_override=(self.career_level.career_level_override(career)))
                    else:
...
class LivesTogetherFilterTerm(InvertibleFilterTerm):
 ...
 L.2218         0  LOAD_FAST                'requesting_sim_info'
                2  LOAD_CONST               None
                4  COMPARE_OP               is
                6  POP_JUMP_IF_FALSE    20  'to 20'

 L.2219         8  LOAD_GLOBAL              FilterResult
               10  LOAD_CONST               0

 L.2220        12  LOAD_FAST                'sim_info'
               14  LOAD_CONST               ('score', 'sim_info')
               16  CALL_FUNCTION_KW_2     2  '2 total positional and keyword args'
               18  RETURN_VALUE     
             20_0  COME_FROM             6  '6'

 L.2222        20  LOAD_FAST                'self'
 ...
 L.2227        86  LOAD_FAST                'requesting_sim_info'
 ...
 L.2228       108  LOAD_FAST                'home_zone_id_a'
              110  POP_JUMP_IF_FALSE   120  'to 120'

 L.2229       112  LOAD_FAST                'home_zone_id_a'
 ...
 L.2231       148  LOAD_FAST                'in_travel_group'
              150  POP_JUMP_IF_TRUE    192  'to 192'
              152  LOAD_FAST                'self'
              154  LOAD_ATTR                consider_hosted_travel_group
              156  POP_JUMP_IF_FALSE   192  'to 192'

 L.2232       158  LOAD_GLOBAL              services
              160  LOAD_METHOD              travel_group_manager
              162  CALL_METHOD_0         0  '0 positional arguments'
              164  LOAD_METHOD              get_travel_group_by_zone_id

 L.2233       166  LOAD_FAST                'requesting_sim_info'
              168  LOAD_ATTR                household
              170  LOAD_ATTR                home_zone_id
              172  CALL_METHOD_1         1  '1 positional argument'
              174  STORE_FAST               'travel_group'

 L.2234       176  LOAD_FAST                'travel_group'
              178  LOAD_CONST               None
              180  COMPARE_OP               is-not
              182  JUMP_IF_FALSE_OR_POP   190  'to 190'
              184  LOAD_FAST                'sim_info'
              186  LOAD_FAST                'travel_group'
              188  COMPARE_OP               in
            190_0  COME_FROM           182  '182'
              190  STORE_FAST               'in_travel_group'
            192_0  COME_FROM           156  '156'
            192_1  COME_FROM           150  '150'

 L.2236       192  LOAD_FAST                'in_household'
              194  POP_JUMP_IF_TRUE    200  'to 200'
              196  LOAD_FAST                'in_travel_group'
              198  JUMP_IF_FALSE_OR_POP   214  'to 214'
            200_0  COME_FROM           194  '194'
              200  LOAD_FAST                'self'
              202  LOAD_ATTR                exclude_sims_in_both
              204  JUMP_IF_FALSE_OR_POP   212  'to 212'
              206  LOAD_FAST                'in_travel_group'
              208  JUMP_IF_FALSE_OR_POP   212  'to 212'
              210  LOAD_FAST                'in_household'
            212_0  COME_FROM           208  '208'
            212_1  COME_FROM           204  '204'
              212  UNARY_NOT        
            214_0  COME_FROM           198  '198'
              214  STORE_FAST               'score'

 L.2238       216  LOAD_GLOBAL              FilterResult
              218  LOAD_FAST                'self'
              220  LOAD_METHOD              invert_score_if_necessary
              222  LOAD_GLOBAL              int
              224  LOAD_FAST                'score'
              226  CALL_FUNCTION_1       1  '1 positional argument'
              228  CALL_METHOD_1         1  '1 positional argument'

 L.2239       230  LOAD_FAST                'sim_info'
              232  LOAD_CONST               ('score', 'sim_info')
              234  CALL_FUNCTION_KW_2     2  '2 total positional and keyword args'
              236  RETURN_VALUE     
               -1  RETURN_LAST      

Parse error at or near `STORE_FAST' instruction at offset 214
...
class CanBeOutside(InvertibleFilterTerm):
    ...
    def calculate_score(self, sim_info, start_time_ticks=None, end_time_ticks=None, **kwargs):
        ...
        if not sim_info.trait_tracker.has_trait(vampire_trait):
            ...
            if not sim_info.age == Age.INFANT:
                if sim_info.age == Age.TODDLER or sim_info.age == Age.CHILD:
                    score = 1
            else:
...
class TunableSimFilter(HasTunableReference, metaclass=TunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.SIM_FILTER)):
    ...
    def create_sim_info(cls, inst, zone_id, blacklist_sim_ids=(), additional_filter_terms=(), sim_constraints=(), **kwargs):
        ...
        household, created_sim_info = household_template.create_household(zone_id, sim_creator=sim_creator, sim_name_type=sim_name_type,
          ...
          sim_debug_name=(template.__name__))
