...
__all__ = [
 ...
 "'urlsplit'", 
 "'urlunsplit'", "'urlencode'", "'parse_qs'", 
 "'parse_qsl'", "'quote'", 
 "'quote_plus'", "'quote_from_bytes'", 
 "'unquote'", "'unquote_plus'", "'unquote_to_bytes'", 
 "'DefragResult'", 
 "'ParseResult'", "'SplitResult'", 
 "'DefragResultBytes'", "'ParseResultBytes'", 
 "'SplitResultBytes'"]
uses_relative = [
...
def unquote_to_bytes(string):
    ...
    if _hextobyte is None:
        _hextobyte = {(a + b).encode(): bytes.fromhex(a + b) for b in _hexdig}
    for item in bits[1:]:
        try:
...
def parse_qsl(qs, keep_blank_values=False, strict_parsing=False, encoding='utf-8', errors='replace'):
    ...
    pairs = [s2 for s1 in qs.split('&') for s2 in iter((s1.split(';')))]
    r = []
